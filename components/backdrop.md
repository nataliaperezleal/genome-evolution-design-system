...
- **Light Theme Context:**
  - Color=Black → dark overlay (palette/alpha/1000, ~70-80% opacity)
  - Color=White → light overlay (palette/alpha/100, ~10% opacity)
- **Dark Theme Context:**
  - Color=Black → dark overlay (palette/alpha/100, ~10% opacity, minimal visibility)
  - Color=White → light overlay (palette/alpha/1000, ~70-80% opacity)
...
### Light Theme
 
| Color | Token | Value | Opacity | Notes |
|-------|-------|-------|---------|-------|
| Black | palette/alpha/1000 | [dark semitransparent] | ~70-80% | Dark overlay, effective blocking |
| White | palette/alpha/100 | [light semitransparent] | ~10% | Light overlay, subtle effect |
 
### Dark Theme
 
| Color | Token | Value | Opacity | Notes |
|-------|-------|-------|---------|-------|
| Black | palette/alpha/100 | [light semitransparent] | ~10% | Dark overlay, minimal visibility |
| White | palette/alpha/1000 | [dark semitransparent] | ~70-80% | Light overlay, effective blocking |
 
**Alpha/Opacity Token Values:**
...
- Backdrop should have no text content (no text to announce)
...
- No contrast requirement between backdrop and floating component (separate layers)
## Content Guidelines
 
...
Color=Black, Light theme, Positioned full-screen, Z-index below modal dialog, Token: palette/alpha/1000 (~70-80% opacity)
 
**Modal in Dark theme (light overlay):**
Color=White, Dark theme, Positioned full-screen, Z-index below modal dialog, Token: palette/alpha/1000 (~70-80% opacity)
 
**Drawer with backdrop (Light theme):**
Color=Black, Light theme, Positioned full-screen, Z-index below drawer panel, Token: palette/alpha/1000
 
**Drawer with backdrop (Dark theme):**
Color=White, Dark theme, Positioned full-screen, Z-index below drawer panel, Token: palette/alpha/1000
 
**Clickable backdrop for dismissal:**
Color=Black (Light theme), Full-screen position, onClick handler triggers modal close, Token: palette/alpha/1000
 
**Non-clickable backdrop:**
Color=Black, Full-screen position, pointer-events: none, Modal dismiss requires button in modal only, Token: palette/alpha/1000
 
**Dialog with backdrop:**
Color=Black (Light theme), Full-screen cover, Modal dialog centered above, Focus trap enabled, Token: palette/alpha/1000
 
**Nested modals with multiple backdrops:**
Each modal has its own backdrop, Z-index stacked appropriately, Token: palette/alpha/1000 for each
 
**Popover with subtle backdrop:**
Color=White (Light theme, subtle), Positioned over affected area only, Non-full-screen backdrop, Token: palette/alpha/100
 
**Mobile modal with backdrop:**
Color=Black, Full viewport coverage, Touching backdrop dismisses modal (optional), Modal animates in from bottom, Token: palette/alpha/1000
 
**Critical action dialog:**
Color=Black (high contrast), Full-screen backdrop, Modal elevated and highlighted, Requires explicit action (not just backdrop click), Token: palette/alpha/1000
 
**Confirmation dialog:**
Color=Black, Full-screen backdrop, Modal with "Confirm" and "Cancel" buttons, Backdrop click optional dismissal, Token: palette/alpha/1000
 
